...
def get_basemap_axes_dims(projection, colorbar=False):
...
def get_normalized_lons_bounds(lons):
...
def get_lon0lat0(lons, lats, norm_max_lons=None, norm_min_lons=None):
    max_lons, min_lons = norm_max_lons, norm_min_lons
    if max_lons is None or min_lons is None:
        max_lons, min_lons = get_normalized_lons_bounds(lons)
...
def get_local_projection_map_dimens(lons, lats):  # , fig_size_in_inches, colorbar=False):
...
#         lon_0 -= 360
    ...
    lon_0, lat_0 = get_lon0lat0(lons, lats, max_lons, min_lons)

#     deg2m_lat = 2 * np.pi * 6371 * 1000 / 360
#     deg2m_lon = deg2m_lat * np.cos(lat_0 / 180 * np.pi)

    deg2m_lon, deg2m_lat = deg2m(lat_0)
...
def adjust_to_fig_dims(width, height,
                       fig, colorbar=False):
...
def get_map_parallels(width, height, lat_0, deg2m_lat):
...
def get_map_meridians(width, height, lon_0, deg2m_lon):
    N2 = int(np.ceil(width / max(width, height) * 8))
...
class LocProjMapHandler(object):
...
#                            alpha=0.5)
            ...
            width, height = \
                get_local_projection_map_dimens(lons, lats)

            width, height = adjust_to_fig_dims(width, height, fig, False)

            lon_0, lat_0 = get_lon0lat0(lons, lats)
            deg2m_lon, deg2m_lat = deg2m(lat_0)
            ...
            parallels = get_map_parallels(width, height, lat_0, deg2m_lat)
...
#                 lon_0 %= 360
            ...
            meridians = get_map_meridians(width, height, lon_0, deg2m_lon)
